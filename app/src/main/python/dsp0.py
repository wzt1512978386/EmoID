...
#     return energy
def energy(signal,kernel_size,stride,padding = 0,startwithzero = False,keeplength=False):
    length = len(signal)
    _length = length+padding
    _signal = np.zeros(_length)

    #pad
    left = int(padding/2)
    right = int(padding/2)+length
    _signal[left:right] = signal
    _signal[0:left] = (signal[:left])[::-1]
    _signal[right:] = (signal[(length-(_length-right)):])[::-1]
    signal = _signal
